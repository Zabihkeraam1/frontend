...
const EditForm = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-indigo-300 via-purple-300 to-pink-300 py-4">
      {/* فرم اصلی */}
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl p-10" dir="rtl">
        ...
        <form className="grid grid-cols-2 gap-4">
          ...
          {[
            ...
            { id: 'last-name', label: 'فامیلی' },
            { id: 'faculty', label: 'دانشکده' },
            { id: 'department', label: 'دیپارتمنت' },
            { id: 'main-residence', label: 'سکونت اصلی' },
            { id: 'current-residence', label: 'سکونت فعلی' },
            { id: 'phone-number', label: 'شماره تلفن' },
            { id: 'id-card', label: 'شماره شناسنامه' },
            ...
            <div key={id} className="relative col-span-2 md:col-span-1">
              <input
                type={type}
                id={id}
                className="block w-full px-3 py-3 text-base text-gray-800 bg-transparent border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-0 focus:border-blue-500 peer"
                placeholder=" "
              />
              <label
                htmlFor={id}
                className="absolute text-base text-gray-600 duration-300 transform -translate-y-4 scale-75 top-3 right-3 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-5"
              >
                {label}
              </label>
            </div>
